...
pub enum RenderError {
    ...
    /// 描いた結果が大きさの上限を超える。
    ///
    /// 上限は 2 つある。投入前に見るのはシーンの解像度から求めた非圧縮サイズ
    /// であり、書き出す前に見るのは符号化後のサイズである。**どちらも要求元が
    /// 選んだ値では決まらない**ため、要求の誤りとしては返さない。
    #[error("レンダリング結果の大きさが上限を超えています")]
    FrameTooLarge,
    /// 完了コールバックが期限内に来なかった。
