...
//! 添えて返す。切り出しは応答側で行う。
//!
//! ただし、1 件あたりに別の読み取りを要する列挙は、切り出しをこの層で行う。
//! オブジェクトは alias と配下 effect を、パレットは 64 件の色を、effect は
//! 設定項目の列挙を、それぞれ 1 件ごとに要する。全件を読んでから切り出すと、
//! 1 要求あたりの重い読み取りが要求ページではなく登録数やプロジェクトの規模で
//! 決まってしまう。窓を先に決めれば、その費用が要求ページで上限付きになる。
...
pub trait ReadAdapter: Send + Sync {
    ...
    /// 掛けない。
    ///
    /// **費用は要求した名前の数で上限が付く。** 全 effect の列挙は登録の有無を
    /// 判定するために 1 度だけ行い、設定項目の列挙は見つかった名前についてだけ
    /// 行う。上限どおりの 10 件をすべて登録済みの名前で埋めた要求で、SDK の
    /// 呼び出しは全 effect の列挙 1 回と設定項目の列挙 10 回になる。説明は SDK を
    /// 通らない。
    ///
